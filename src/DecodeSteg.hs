...
readBitOfImage img bitsPerPixel byteIdx bitIdx offset period
  ...
  where 
    a = floor (fromIntegral (byteIdx * 8 + bitIdx) * period)
    pixIdx = div a (bitsPerPixel * 3) + offset 
    px = mod pixIdx (getWidth img)
    py = div pixIdx (getWidth img)
    color = div (mod a (bitsPerPixel * 3)) bitsPerPixel
    lsbIdx = mod (mod a (bitsPerPixel * 3)) bitsPerPixel
